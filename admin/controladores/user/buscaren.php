    ...
    <table style="width:100%">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $sql2 = "SELECT * FROM correos WHERE cor_eliminado='N' AND cor_usu_remitente=$codigoAdmin AND cor_usu_destinatario=$row[usu_codigo];";
                $result2 = $conn->query($sql2);
                $row2 = $result2->fetch_assoc();
                $sqlcorreo = "SELECT usu_correo FROM usuario WHERE usu_codigo =$row2[cor_usu_destinatario]";
